...
public class EtceteraBinding
{
    ...
    public static IEnumerator takeScreenShot( string filename )
    {
		...
		// wait for the file to get written or 30 * fps frames max
		var path = Application.persistentDataPath + "/" + filename;
		var frame = 0;
		var framesToWaitFor = 300;
		if( Application.targetFrameRate > 15 )
			framesToWaitFor = 30 * Application.targetFrameRate;
		
		while( !System.IO.File.Exists( path ) && frame < framesToWaitFor )
		{
			yield return null;
			++frame;
		}
		
		// put in a delay for slow iPad 3 images due to Unity not appearing to write atomically
		if( iPhone.generation == iPhoneGeneration.iPad3Gen || iPhone.generation == iPhoneGeneration.iPadUnknown )
			yield return new WaitForSeconds( 1 );
		
		// now make sure the file has some girth
		long size = 0;
		while( true )
		{
    ...
    public static void showAlertWithTitleMessageAndButtons( string title, string message, string[] buttons )
    {
        if( Application.platform == RuntimePlatform.IPhonePlayer )
		{
			...
			_etceteraShowAlertWithTitleMessageAndButtons( title, message, Prime31.Json.jsonEncode( buttonArrayList ) );
		}
    }
    ...
    {
        if( Application.platform == RuntimePlatform.IPhonePlayer )
			_etceteraShowSMSComposer( Prime31.Json.jsonEncode( recipients ), body );
    }
