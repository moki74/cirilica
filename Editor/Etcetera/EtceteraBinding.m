...
{
	NSArray *recipientArray = [GetStringParam( recipients ) objectFromJSONString];
...
{
	if( [UnityAppController respondsToSelector:@selector(registerForRemoteNotificationTypes:)] )
		[UnityAppController performSelector:@selector(registerForRemoteNotificationTypes:) withObject:[NSNumber numberWithInt:types]];
	else
		NSLog( @"UnityAppController does not respond to registerForRemoteNotificationTypes:. Did you remove the UnityUnityUnityAppControllerPushAdditions file and forget to readd it?" );
}


int _etceteraGetEnabledRemoteNotificationTypes()
{
	if( [UnityAppController respondsToSelector:@selector(enabledRemoteNotificationTypes)] )
	{
		NSNumber *num = [UnityAppController performSelector:@selector(enabledRemoteNotificationTypes)];
		return [num intValue];
	}
	else
	{
		NSLog( @"UnityUnityUnityAppController does not respond to enabledRemoteNotificationTypes. Did you remove the UnityUnityUnityAppControllerPushAdditions file and forget to readd it?" );
	}
